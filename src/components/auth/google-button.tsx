"use client";

import { useState } from "react";
import { Button } from "@/components/ui/button";
import { createClient } from "@/lib/supabase/client";

/** Logo oficial do Google — as 4 cores da marca não podem ser alteradas. */
function GoogleG({ size = 18 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" aria-hidden="true" focusable="false">
      <path
        fill="#FFC107"
        d="M43.611 20.083H42V20H24v8h11.303c-1.649 4.657-6.08 8-11.303 8-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4 12.955 4 4 12.955 4 24s8.955 20 20 20 20-8.955 20-20c0-1.341-.138-2.65-.389-3.917z"
      />
      <path
        fill="#FF3D00"
        d="M6.306 14.691l6.571 4.819C14.655 15.108 18.961 12 24 12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4 16.318 4 9.656 8.337 6.306 14.691z"
      />
      <path
        fill="#4CAF50"
        d="M24 44c5.166 0 9.86-1.977 13.409-5.192l-6.19-5.238C29.211 35.091 26.715 36 24 36c-5.202 0-9.619-3.317-11.283-7.946l-6.522 5.025C9.505 39.556 16.227 44 24 44z"
      />
      <path
        fill="#1976D2"
        d="M43.611 20.083H42V20H24v8h11.303a12.04 12.04 0 01-4.087 5.571l6.19 5.238C37.271 39.205 44 34 44 24c0-1.341-.138-2.65-.389-3.917z"
      />
    </svg>
  );
}

/**
 * Entrar/cadastrar com Google. Um provider OAuth cobre os dois casos — não há
 * "criar conta com Google" separado: se o e-mail é novo, a conta é criada.
 *
 * O retorno cai em `/auth/callback`, que já troca o `code` por sessão e manda
 * pro `next` (padrão `/dashboard`) — de onde o middleware encaminha pra
 * `/assinar` se ainda não houver assinatura. Nenhuma mudança lá foi necessária.
 */
export function GoogleButton({ onErro }: { onErro: (msg: string) => void }) {
  const [carregando, setCarregando] = useState(false);

  async function onClick() {
    setCarregando(true);
    const supabase = createClient();
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: `${window.location.origin}/auth/callback` },
    });
    // Em caso de sucesso o browser é redirecionado pro Google e este componente
    // é descartado — só chega aqui se falhou (provider desabilitado, rede etc).
    if (error) {
      setCarregando(false);
      onErro("Não deu pra abrir o login do Google. Tenta de novo.");
    }
  }

  return (
    <Button
      type="button"
      variant="outline"
      onClick={onClick}
      disabled={carregando}
      className="h-12 w-full gap-2.5 rounded-xl text-[15px] font-bold"
    >
      <GoogleG />
      {carregando ? "Abrindo..." : "Continuar com Google"}
    </Button>
  );
}
